...
def test_bans_are_not_doubled_when_champion_has_two_roles(monkeypatch):
    ...
    stats, bans, _, _ = run_matches(monkeypatch, [match])
...
def test_empty_ban_slot_is_ignored(monkeypatch):
    ...
    _, bans, _, _ = run_matches(monkeypatch, [match])
...
def test_participant_without_lane_is_skipped(monkeypatch):
    ...
    stats, _, _, _ = run_matches(monkeypatch, [match])
...
def test_unusable_game_version_skips_only_that_match(monkeypatch, version):
    ...
    stats, _, total, patch = run_matches(monkeypatch, [good, bad])
...
def test_malformed_match_does_not_abort_the_run(monkeypatch):
    ...
    stats, _, _, _ = run_matches(monkeypatch, [broken, good])
...
def test_matches_from_another_patch_are_excluded(monkeypatch):
    """
    Documents current behaviour: the first match decides the patch, and
    anything from a different one is dropped. On patch-release day this
    silently discards part of the data - see the README's known issues.
    """
    matches = [
    ...
    _, _, total, patch = run_matches(monkeypatch, matches)
